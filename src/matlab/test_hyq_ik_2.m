...
% - single foot position constraint in air (lf)
% - 3 foot position constraints on the ground
% - quasi-static using 3 feet (not the lf) with point contacts
% - constraint on the rpy of the trunk
...
position_constraint_2_lh = WorldPositionConstraint(r, links.lh_foot, point_in_link_frame, lower_bounds, upper_bounds, [0.0, 1.0]);quaternion_constraint_2 = WorldQuatConstraint(r, links.lh_foot, xyz_quat(4:7), 0.0017453292519943296, [0.0, 1.0]);
send_bot_pose(xyz_quat(1:3), xyz_quat(4:7), 'POSE_BODY' );
...
lower_bounds = xyz_quat(1:3) + [-0.0001; -0.0001; -0.0001];
upper_bounds = xyz_quat(1:3) + [0.0001; 0.0001; 0.0001];
position_constraint_2_rh = WorldPositionConstraint(r, links.rh_foot, point_in_link_frame, lower_bounds, upper_bounds, [0.0, 1.0]);quaternion_constraint_2 = WorldQuatConstraint(r, links.lh_foot, xyz_quat(4:7), 0.0017453292519943296, [0.0, 1.0]);
...
lower_bounds = xyz_quat(1:3) + [-0.0001; -0.0001; -0.0001];
upper_bounds = xyz_quat(1:3) + [0.0001; 0.0001; 0.0001];
position_constraint_2_rf = WorldPositionConstraint(r, links.rf_foot, point_in_link_frame, lower_bounds, upper_bounds, [0.0, 1.0]);quaternion_constraint_2 = WorldQuatConstraint(r, links.lh_foot, xyz_quat(4:7), 0.0017453292519943296, [0.0, 1.0]);
...
posture_constraint_3_orient = PostureConstraint(r, [-inf, inf]);
...
posture_constraint_3_orient = posture_constraint_3_orient.setJointLimits(joint_inds, joints_lower_limit, joints_upper_limit);
...
active_constraints = {qsc_constraint_0,position_constraint_1, position_constraint_2_lh, position_constraint_2_rf, position_constraint_2_rh, posture_constraint_3_orient};
