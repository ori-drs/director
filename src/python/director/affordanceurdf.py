...
def geometryFromAffordance(aff):
    ...
    if isinstance(aff, affordanceitems.CylinderAffordanceItem):
        return urdf.Cylinder(
            length=aff.getProperty("Length"), radius=aff.getProperty("Radius")
        )
...
def materialFromAffordance(aff):
    ...
    return urdf.Material(
        name=stringWithAffordanceId("material_%s", aff), color=color, texture=None
    )
...
def linkFromAffordance(aff):
    ...
    return urdf.Link(
        name=stringWithAffordanceId("link_%s", aff), visual=visual, collision=collision
    )
