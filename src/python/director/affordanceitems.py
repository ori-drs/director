...
class AffordanceItem(PolyDataItem):
    ...
    COPY_MODE_SKIP_LOCAL = (
        1  # skips properties that should keep local values such as visibility
    )
    ...
    def __init__(self, name, polyData, view):
        ...
        self.params = {}
        self.addProperty(
            "uuid", newUUID(), attributes=om.PropertyAttributes(hidden=True)
        )
    ...
    def syncProperties(self, desc, copyMode=COPY_MODE_ALL):
        for propertyName, propertyValue in desc.items():
            ...
            if self.hasProperty(propertyName) and (
                self.getProperty(propertyName) != propertyValue
            ):
    ...
    def __init__(self, name, view):
        ...
        self.addProperty(
            ...
            attributes=om.PropertyAttributes(
                decimals=3, singleStep=0.01, minimum=0.0, maximum=1e4
            ),
        )
        self.addProperty(
            "Subdivisions", 0, attributes=om.PropertyAttributes(minimum=0, maximum=1000)
        )
...
class CapsuleRingAffordanceItem(AffordanceItem):
    def __init__(self, name, view):
        ...
        self.addProperty(
            ...
            8,
            attributes=om.PropertyAttributes(
                decimals=3, singleStep=1, minimum=3, maximum=100
            ),
        )
