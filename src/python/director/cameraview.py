...
class CameraView(object):
    def __init__(self, imageManager, view=None, robotName=""):
        ...
        try:
            if self.robotName:
                self.images = [
                    camera["name"]
                    for camera in drcargs.getRobotConfig(self.robotName)["sensors"][
                        "camera"
                    ]["color"]
                ]
    ...
    def getSphereGeometry(self, imageName):
        ...
        self.imageManager.queue[self.robotName][imageName].compute_texture_coords(
            imageName, geometry
        )
        ...
        sphereObj = vis.showPolyData(
            geometry, imageName, view=self.view, parent="cameras"
        )
    ...
    def updateSphereGeometry(self):
        ...
        for imageName in self.images:
            ...
            self.imageManager.queue[self.robotName][
                imageName
            ].get_body_to_camera_transform(transform)
...
class ImageWidget(object):
    ...
    def updateView(self):
        ...
        for imageName in self.imageNames:
            currentUtime = max(
                self.imageManager.updateImage(imageName, self.robotName), currentUtime
            )
...
class CameraImageView(object):
    ...
    def getWorldPositionAndRay(self, imagePixel, imageUtime=None):
        ...
        self.imageManager.queue.getTransform(
            self.imageName, "local", imageUtime, cameraToLocal
        )
