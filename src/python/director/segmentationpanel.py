...
class SegmentationPanel(object):
    ...
    def _makeDebrisWizard(self):
        ...
        self.lockAffordanceButton = _makeButton(
            "lock affordance to hand", self.onLockAffordanceToHand
        )
    ...
    def _makeDoorWizard(self):
        ...
        l.addWidget(
            _makeButton(
                "segment door handle - left",
                functools.partial(startDoorHandleSegmentation, "door_handle_left"),
            )
        )
        l.addWidget(
            _makeButton(
                "segment door handle - right",
                functools.partial(startDoorHandleSegmentation, "door_handle_right"),
            )
        )
        l.addWidget(
            _makeButton(
                "segment door frame",
                functools.partial(startDoorHandleSegmentation, "doorframe"),
            )
        )
        l.addStretch()
        return wizard
    ...
    def _makeValveWizard(self):
        ...
        hl.setMargin(0)
        hl.addWidget(
            _makeButton("segment value radius:", self.segmentValveCustomRadius)
        )
    ...
    def _makeDrillWizard(self):
        ...
        l.addWidget(
            _makeButton(
                "segment drill aligned with table",
                startDrillAutoSegmentationAlignedWithTable,
            )
        )
        l.addWidget(
            _makeButton(
                "segment target using wall center", segmentDrillWallFromWallCenter
            )
        )
        ...
        hl = QtGui.QHBoxLayout(hw)
        hl.setMargin(0)
        self.moveDrillToHandButton = _makeButton(
            "move drill to hand", self.onMoveDrillToHandClicked
        )
        ...
        hw.connect(
            self.drillDepthOffsetSlider, "valueChanged(int)", self.moveDrillToHand
        )
        hw.connect(
            self.drillLateralOffsetSlider, "valueChanged(int)", self.moveDrillToHand
        )
        l.addWidget(hw)
    ...
    def _makeTerrainWizard(self):
        ...
        self.cinderBlockButton.connect(
            "clicked()",
            functools.partial(
                self.onTerrainCinderblockSelected, self.cinderBlockButton
            ),
        )
        self.cinderBlock2Button.connect(
            "clicked()",
            functools.partial(
                self.onTerrainCinderblockSelected, self.cinderBlock2Button
            ),
        )
        ...
        l.addWidget(
            _makeButton(
                "double wide",
                functools.partial(startInteractiveLineDraw, [0.1905 * 2, 0.149225]),
            )
        )
    ...
    def moveDrillToHand(self):
        ...
        self.drillOffset = getDrillInHandOffset(
            zRotation=rotation,
            zTranslation=offset,
            xTranslation=depthoffset,
            yTranslation=lateralOffset,
            flip=flip,
        )
...
def getOrCreateSegmentationView():
    ...
    if not segmentationView:
        segmentationView = viewManager.createView(
            "Segmentation View", "VTK View", 1, ""
        )
...
def activateSegmentationMode(polyData=None, debug=False):
    ...
    if doRemoveGround:
        ...
        segmentationObj = updatePolyData(
            groundPoints, "ground", alpha=0.3, visible=False
        )

    segmentationObj = updatePolyData(polyData, "pointcloud snapshot", alpha=0.3)
    segmentationObj.headAxis = perception._multisenseItem.model.getAxis(
        "head", [1, 0, 0]
    )
