...
class SceneLoader(object):
    ...
    def generateBoxLinkNode(self, aff):
        ...
        pose = np.append(
            ...
            transformUtils.quaternionToRollPitchYaw(aff.getDescription()["pose"][1]),
        )
    ...
    def generateCylinderLinkNode(self, aff):
        ...
        if aff.getDescription()["Collision Enabled"]:
            ...
            link.find("collision/geometry/cylinder/radius").text = "{:.4f}".format(
                aff.getDescription()["Radius"]
            )
            link.find("collision/geometry/cylinder").append(etree.Element("length"))
            link.find("collision/geometry/cylinder/length").text = "{:.4f}".format(
    ...
    def generateSDFfromAffordances(self):
        ...
        for aff in affordances:
            if aff.getDescription()["classname"] in [
                "BoxAffordanceItem",
                "CylinderAffordanceItem",
            ]:
