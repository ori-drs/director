...
def addSubscriber(
    ...
    def handleMessage(messageData, channel):
        ...
        try:
            ...
            if historicalLoader is not None:
                # try:
                msg = historicalLoader.decode(
                    messageClass.__module__.split(".")[-1], messageData.data()
                )
                loadSuccessful = True
            # except ValueError:
            #     pass
    ...
    if callback is not None:
        if messageClass is not None:
            subscriber.connect(
                "messageReceived(const QByteArray&, const QString&)", handleMessage
            )
        else:
            subscriber.connect(
                "messageReceived(const QByteArray&, const QString&)", callback
            )
    else:
...
class MessageResponseHelper(object):
    ...
    @staticmethod
    def publishAndWait(
        channel, message, responseChannel, responseMessageClass=None, timeout=5000
    ):
...
class MessageCollector(object):
    ...
    def start(self):
        if not self.subscriber:
            self.subscriber = addSubscriber(
                self.channel, messageClass=self.messageClass, callback=self.onMessage
            )
...
class HistoricalLCMLoader(object):
    ...
    def buildTypeAtSHA(self, type_name, sha):
        ...
        subprocess.check_call(
            "lcm-gen --lazy -p --ppath {build:s} {source:s}".format(
                build=sha_build_dir, source=os.path.join(sha_source_dir, "*")
            ),
            shell=True,
        )
        build_files = [
            f
            for f in os.listdir(os.path.join(sha_build_dir, self.package_name))
            if f.endswith(".py")
        ]
        build_type_names = [t.replace(".py", "") for t in build_files]
        for f in build_files:
            subprocess.check_call(
                r"perl -ne 's/{pkg:s}(?=\.({type_list:s}[^a-zA-Z0-9_]))/{pkg:s}{sha:s}/g; print;' < {infile:s} > {outfile:s}".format(
    ...
    def getOrCreateSourceFiles(self, type_name, sha, recursive=False):
        ...
        if not os.path.exists(targets[0]):
            try:
                subprocess.check_call(
                    "git -C {base:s} show {sha:s}:{typepath:s} > {fpath:s}".format(
                        base=self.repo_path,
                        sha=sha,
                ...
                raise TypeNotFoundError(
                    "The target LCMtype cannot be found at this revision"
                )
        if recursive:
    ...
    def getChildTypes(self, type_name, sha):
        ...
        children = []
        debug_data = subprocess.check_output(
            "lcm-gen -d {fpath:s}".format(fpath=source_file), shell=True
        )
        debug_lines = debug_data.split("\n")
        for line in debug_lines:
            line = line.lstrip()
            match = re.match(
                r"{pkg:s}\.(?P<childname>[^\s]+)".format(pkg=self.package_name), line
            )
            if match:
    ...
    def getSHAsForType(self, type_name):
        ...
        """
        relative_type_path = os.path.join(
            self.lcmtypes_path,
            "{package_name:s}_{type_name:s}.lcm".format(
                package_name=self.package_name, type_name=type_name
            ),
        )
        cdata = subprocess.check_output(
            "git --no-pager -C {0:s} log --pretty=oneline {1:s}".format(
                self.repo_path, relative_type_path
            ),
            shell=True,
        )
            ...
            try:
                for child in self.getChildTypes(type_name, sha):
                    if (child, sha) not in processed:
                        ...
                        child_shas.update(
                            self.getSHAsForTypeAndChildren(child, processed)
                        )
    ...
    def decode(self, type_name, msg_data):
        ...
        if not self._initialized:
            print(
                "Warning: Possible out-of-date LCM message received. I will now try to decode the message using older versions of the type definition. This will be slow the first time it happens."
            )
            self._initialized = True
        i = 0
        if not type_name in self._mru_shas_cache:
            self._mru_shas_cache[type_name] = list(
                self.getSHAsForTypeAndChildren(type_name)
            )
        for i, sha in enumerate(self._mru_shas_cache[type_name]):
            ...
            return msg_obj
        raise ValueError(
            "Unable to decode message data with any available type definitions."
        )
...
class LCMLoggerManager(object):
    ...
    def getActiveLogFilenames(self):
        files = []
        for pid, command in self.existingLoggerProcesses.items():
