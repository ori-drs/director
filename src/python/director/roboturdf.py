...
class RobotModelItem(om.ObjectModelItem):
    ...
    ):  # lcmdrc.footstep_params_t.SUPPORT_GROUPS_HEEL_TOE):
        """
    ...
    def getFeetMidPoint(self, useWorldZ=True):
        ...
        contact_pts_mid_left = np.mean(
            contact_pts_left, axis=0
        )  # mid point on foot relative to foot frame
        contact_pts_mid_right = np.mean(
            contact_pts_right, axis=0
        )  # mid point on foot relative to foot frame
        ...
        if useWorldZ:
            rpy = [
                0.0,
                0.0,
                np.degrees(transformUtils.rollPitchYawFromTransform(t_feet_mid)[2]),
            ]
...
def onModelPublisherString(msg):
    ...
    if lastStr is not None:
        app.showInfoMessage(
            "A model publisher string was received that differs from the previous string. "
...
class HandFactory(object):
    ...
    def placeHandModelWithTransform(
        self, transform, view, side, name=None, parent=None
    ):
...
class HandLoader(object):
    def __init__(self, handType, robotModel):
        ...
        handCombinations = drcargs.getRobotConfig(robotModel.robotName)[
            "handCombinations"
        ]
    ...
    def initHandTransforms(self, robotModel, thisCombination):
        ...
        robotMountToPalm = self.getLinkToLinkTransform(
            robotModel, robotMountLink, palmLink
        )
    ...
    def getHandUrdfFullPath(self):
        if director.getDRCBaseIsSet():
            urdfBase = os.path.join(
                getDRCBaseDir(), "params/models/common_components/hand_factory"
            )
        else:
    ...
    def newPolyData(self, name, view, parent=None):
        ...
        obj = vis.showPolyData(
            polyData, name, view=view, color=color, visible=False, parent=parent
        )
        obj.side = self.side
        ...
        frameObj = vis.showFrame(
            frame, "%s frame" % name, view=view, scale=0.2, visible=False, parent=obj
        )
        return obj
    ...
    def moveHandModelToFrame(model, frame):
        ...
        model.model.setJointPositions(
            pose, ["base_x", "base_y", "base_z", "base_roll", "base_pitch", "base_yaw"]
        )
...
def setRobotiqJoints(robotModel, side, fingers=[0.0, 0.0, 0.0], palm=[0.0, 0.0, 0.0]):
    robotModel.model.setJointPositions(
        np.tile(fingers, 3),
        [
            "%s_finger_%s_joint_%d" % (side, n, i + 1)
            for n in ["1", "2", "middle"]
            for i in range(3)
        ],
    )
    robotModel.model.setJointPositions(
        palm, ["%s_palm_finger_%s_joint" % (side, n) for n in ["1", "2", "middle"]]
    )
