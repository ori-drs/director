...
TreeFkSolverPosFull_recursive::TreeFkSolverPosFull_recursive(const KDL::Tree& _tree):
  tree(_tree)
{
}
...
void TreeFkSolverPosFull_recursive::addFrameToMap(const map<string, double>& q_in, 
						  ...
						  const KDL::SegmentMap::const_iterator this_segment,
						  bool flatten_tree)
{
  ...
  double jnt_p = 0;
  if (this_segment->second.segment.getJoint().getType() != KDL::Joint::None){
  ...
  for (vector<KDL::SegmentMap::const_iterator>::const_iterator child=this_segment->second.children.begin(); child !=this_segment->second.children.end(); child++){
    if (flatten_tree)
